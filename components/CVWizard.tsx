...
type WizardStep = 'upload' | 'job-description' | 'ai-enhance' | 'template-select' | 'download';
...
export default function CVWizard() {
  ...
  const [jobDescription, setJobDescription] = useState<JobDescription>({ description: '' });
  ...
  const nextStep = () => {
    const steps: WizardStep[] = ['upload', 'job-description', 'ai-enhance', 'template-select', 'download'];
    ...
    <div className={`min-h-screen ${isDark ? 'bg-[#0a0b0d]' : 'bg-gray-50'} transition-colors duration-300`}>
          ...
          <div className="max-w-7xl mx-auto px-6 py-4">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <h3 className={`text-sm font-bold ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
            ...
            <div className="flex gap-2">
              {['upload', 'job-description', 'ai-enhance', 'template-select'].map((step, index) => {
                const stepIndex = ['upload', 'job-description', 'ai-enhance', 'template-select'].indexOf(currentStep);
      ...
      {showApiKeyInput && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className={`max-w-2xl w-full rounded-2xl ${isDark ? 'bg-gray-900' : 'bg-white'} p-6 max-h-[90vh] overflow-y-auto`}>
            ...
            <ApiKeyInput theme={theme} onApiKeySet={() => setShowApiKeyInput(false)} />
          </div>
        </div>
      )}
