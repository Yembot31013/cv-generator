...
interface AIEnhancementStepProps {
  ...
  uploadedFiles?: File[];
  onEnhanced: (data: CVData, coverLetter?: string) => void;
  onBack: () => void;
  onNext: () => void;
  theme?: "dark" | "light";
}
  ...
  const startEnhancement = async () => {
    ...
    try {
      ...
      const enhanced = jobDescription.description
        ? await enhancer.enhanceCV(partialCV, jobDescription, uploadedFiles)
        : await enhancer.quickEnhance(partialCV, uploadedFiles);

      setProgress(60);

      // Generate cover letter if job description is provided
      let coverLetterText: string | undefined;
      if (
        jobDescription.description &&
        jobDescription.description.length > 50
      ) {
        try {
          const coverLetter = await enhancer.generateCoverLetter(
            enhanced,
            jobDescription,
            uploadedFiles
          );
      ...
      onEnhanced(enhanced, coverLetterText);
        ...
        {status === "processing" && (
          <div className="text-center">
            ...
            >
              ...
              {progress >= 60 &&
                progress < 85 &&
                "📝 Adding metrics and achievements..."}
              {progress >= 85 &&
                progress < 100 &&
                "✉️ Generating personalized cover letter..."}
              {progress >= 100 && "🎨 Finalizing your perfect CV..."}
            </div>
          </div>
        )}
